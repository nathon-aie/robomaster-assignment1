...
└── tests/                        # ชุด Automated Unit & Integration Tests (21/21 Pass 100%)
    ...
    └── test_panel.py             # ทดสอบ Mission Control: แผนที่/A*/mapping/tracking/gripper/UI
```
...
> - เครื่องมือ `Place` + ปุ่ม `FETCH`/`DELIVER`: มือว่าง=ไปหนีบขวดที่ Goal,
>   ถืออยู่=เอาไปวางที่จุด `P` หันตามทิศที่กำหนด (ถือของอยู่จะไม่สนเซนเซอร์หน้า)
> - `PLACE TARGET`: เปิดหน้าต่างซูมช่อง Place แบ่ง 5×5 เลือกจุดวางละเอียดในช่อง
> - `BACK TO START`: ขับกลับช่อง Start จากตำแหน่งไหนก็ได้
