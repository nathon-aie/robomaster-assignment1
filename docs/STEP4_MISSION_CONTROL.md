...
### ปุ่ม `OBJECT PLACE` — ไปหาของแล้วหยิบ

```text
ไปที่ช่องที่ตั้งเป็น Goal -> จอด "ข้าง ๆ" ช่องนั้น (ของกินพื้นที่ช่อง)
   -> หมุนตัวกวาด ToF ทีละ 90 องศา จนกว่าจะเจอของอยู่ตรงหน้า -> หนีบ
```

ToF ยึดติดกับตัวถัง ไม่ได้หมุนเองได้ — "เล็งเซนเซอร์" จึงแปลว่า **หมุนตัวหุ่น**
กวาดได้สูงสุด 4 ครั้ง (ครบวง, ปรับที่ `object_scan_turns`) หมุนทีละ 90 องศา
เพราะ detector เชื่อถือได้เฉพาะตอนหันตรงแกน — แผนที่รู้ว่าลำแสงควรไปโดนอะไร
หากกวาดครบวงแล้วไม่เจอ จะรายงาน `NOTHING FOUND` ไม่ใช่หนีบลม

จบแล้วสถานะเป็น `HOLDING` รอกด DELIVERY

### ทำไมบางทีหุ่นหมุนหาไม่เจอ (และแก้ยังไง)

ของที่ตั้งบนพื้นมี 2 กรณีที่ ToF ไม่มีทางยืนยันได้:

| กรณี | ToF อ่านได้ | ผลเดิม |
| --- | --- | --- |
| ของเตี้ย ลำแสงผ่านข้ามหัวไปโดนกำแพงข้างหลัง | 0.86 m | `out of detection range` → หมุนวน |
| AUTO MAP เคยบันทึกของชิ้นนั้นเป็น "กำแพง" | แผนที่บอก 0.30 m | `matches the mapped wall distance` → หมุนวน |

แก้โดย **เชื่อ marker ที่คนวางไว้มากกว่าแผนที่**:

* ถ้าลำแสงชี้เข้าช่องที่ตั้ง `Object` ไว้ → ข้ามการเทียบกับแผนที่ไปเลย
  (แผนที่เป็นพยานที่เชื่อถือได้น้อยกว่าคน โดยเฉพาะเมื่อของนั้นเองคือสิ่งที่ถูก map เป็นกำแพง)
* กวาดแล้วยังไม่เจอ → หันกลับไปหาช่องที่ mark ไว้ แล้ว **หนีบเลย**
  พร้อมเขียน log ตรง ๆ ว่า *"ToF did not confirm anything ... Trusting the Object marker"*
  (ปิดได้ที่ `trust_object_marker`)
* ไม่หมุนมั่วก่อน — `_approach` หันเข้าช่องที่ mark ไว้อยู่แล้ว จึงเช็คทิศนั้นก่อนเสมอ

ถ้าช่องนั้นว่างจริง ๆ จะจบที่ `PICK FAILED` (คีบแล้วไม่มีอะไร) ซึ่งตรงกับความจริง
มากกว่าจะบอกว่า "หาไม่เจอ"

### ปุ่ม `DELIVERY` — เอาของที่ถืออยู่ไปวางที่จุดเล็ง

```text
ขับไปที่ Place point -> หันตามทิศที่กำหนด -> ขยับเข้าจุดย่อยที่เลือก -> ปล่อย
```

ต้องมีของในมือถึงจะกดได้ (ปุ่มจะจางถ้ามือว่าง) และต้องตั้งจุด Place ไว้ก่อน

### เครื่องมือ `Object` และ `Delivery` (อยู่ในแถว TOOLS)

| เครื่องมือ | ตั้งอะไร | บนแผนที่ |
| --- | --- | --- |
| `Object` | ช่องที่ของวางอยู่ (`object_cell`) | ช่องเขียว `O` (คลิกซ้ำ = ลบ) |
| `Delivery` | ช่องที่จะเอาไปวาง + ทิศที่หันตอนวาง | ช่องม่วง `P` + ลูกศรทิศ (คลิกซ้ำ/คลิกขวา = หมุนทิศ) |

ทั้งสองช่องมี **จุดเล็งย่อย** แสดงเป็นจุดกลมเล็กในช่อง

### Object marker = ของจริงในโลกจำลอง

การกดวาง `Object` คือการบอกว่า "ของตั้งอยู่ตรงนั้นจริง ๆ" (บนสนามจริงคือคนไปวางขวดไว้)
ตัว simulator จึงต้อง **สร้างของขึ้นมาจริง** ในโลกจำลองด้วย ไม่งั้น ToF กวาดไปเจอช่องว่าง
แล้วรายงาน `NOTHING FOUND` ทั้งที่แผนที่แสดงว่ามีของอยู่

* เครื่องมือ `Object` ใส่ทั้ง `object_cell` และเพิ่มลง `grid.objects`
* `ground_truth_from()` ยังเผื่อไว้อีกชั้น — แผนที่ที่ตั้ง marker มาจากไฟล์/โค้ด
  ก็จะมีของจริงในโลกจำลองเสมอ

### แสดงสถานะ gripper และของที่ถืออยู่

| ที่ | แสดงอะไร |
| --- | --- |
| chip `GRIPPER` บนหัวจอ | `EMPTY` / `HOLDING OBJECT` (สีฟ้าเมื่อถืออยู่) |
| แถว `GRIPPER` ในแผง ROBOT STATE | เหมือนกัน (อยู่บน ๆ ของรายการ) |
| บนแผนที่ | วงกลมสีฟ้าติดหน้าหุ่น **เคลื่อนและหมุนไปกับหุ่น** |

chip อยู่บนหัวจอเพราะแผง ROBOT STATE เป็นอันแรกที่โดนตัดแถวเมื่อหน้าต่างเตี้ย
ของที่ถืออยู่จะไม่หายไปจากจอระหว่างขนย้าย

### หน้าต่างเล็งจุดย่อย 9×9 — `TARGET: OBJECT` / `TARGET: DELIVERY`

ช่องกริดกว้าง 60 ซม. แต่ขวดกว้างไม่กี่ ซม. — ปุ่ม TARGET เปิดหน้าต่างซูมช่องเดียว
แบ่งเป็น **9×9** (~6.7 ซม./ช่องย่อย บนกริด 60 ซม.) คลิกช่องย่อยเพื่อเล็ง

| ปุ่ม | เล็งอะไร | มีทิศไหม |
| --- | --- | --- |
| `TARGET: OBJECT` | ตำแหน่งจริงที่ของวางอยู่ในช่อง (`object_offset`) | ไม่มี — ของแค่ตั้งอยู่เฉย ๆ |
| `TARGET: DELIVERY` | ตำแหน่งที่จะปล่อยของในช่อง (`delivery_offset`) | มี ปุ่ม `TURN` ตั้งทิศที่หุ่นหันตอนปล่อย |

* เก็บเป็นเศษส่วนของช่อง แกนจอ (+x ตะวันออก / +y ใต้) → หมุนตามแผนที่, บันทึกลง JSON
* ตอนวางจริง `delivery_offset` แปลงเป็นเฟรมหุ่น (เดินหน้า, ขวา) แล้วขยับ chassis ก่อนปล่อย
* ปุ่ม `CENTRE` รีเซ็ตกลางช่อง

ไฟล์แผนที่เก่าที่ใช้คีย์ `"place"` ยังโหลดได้ (อ่านทั้ง `"delivery"` และ `"place"`)
และแผนที่ที่ยังไม่ได้ตั้ง Object marker จะ fallback ไปใช้ช่อง Goal พร้อมเตือนใน log
